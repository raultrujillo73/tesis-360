---
id: F1
order: 1
title: CONSTRUIR LA BIBLIOTECA BIBLIOGRÁFICA
status: en_curso
objective: reunir las fuentes que sustentarán la investigación.
tasks:
  - id: F1-T01
    title: Descargar bibliografía internacional prioritaria
    status: en_curso
    priority: P0
    responsible: R
    deliverable: null
    completion_criteria: primera ronda disponible para análisis.
    notes: |-
      - **Lista inicial:** Shinde et al. 2023; Brilakis et al. 2005; Brilakis & Soibelman 2006; Wu & Tory/PhotoScope; Eiris et al. 2017; Eiris & Gheisari 2019; Fobiri et al. 2022; Pal & Hsieh 2021; Erazo-Rondinel & Melgar 2024; Taha et al. 2025; Adegoke et al. 2026.
      - **Buscar primero en:** fuente oficial, OpenAlex, repositorio institucional, versión OA/preprint legítima.
  - id: F1-T02
    title: Descargar tesis peruanas prioritarias
    status: en_curso
    priority: P0
    responsible: R
    deliverable: null
    completion_criteria: null
    notes: "- **Fuentes:** UNI, PUCP, UPC, ALICIA/CONCYTEC."
  - id: F1-T03
    title: Descargar tesis UNI como referencia metodológica/formal
    status: pendiente
    priority: P1
    responsible: R
    deliverable: null
    completion_criteria: null
    notes: |-
      - **Incluir inicialmente:** microsimulación BRT; pavimentos Aeropuerto de Iquitos; planificación de refinería mediante LPS; diseño/construcción virtual para suministro de acero; otras FIC comparables.
      - **Objetivo:** estudiar cómo delimitan caso, objetivos, metodología, aplicación y conclusiones.
  - id: F1-T04
    title: Crear convención de nombres
    status: pendiente
    priority: null
    responsible: R
    deliverable: null
    completion_criteria: null
    notes: "- **Ejemplo:** `2023_Shinde_360_AEC_Systematic_Review.pdf`."
  - id: F1-T05
    title: Crear matriz maestra de bibliografía
    status: pendiente
    priority: null
    responsible: R+GPT
    deliverable: null
    completion_criteria: null
    notes: "- **Campos:** ID, referencia, año, tipo, revista/universidad, DOI, OpenAlex, OA/PDF, tema, estado de lectura, ficha, capítulo/uso previsto, prioridad."
---

# F1 — CONSTRUIR LA BIBLIOTECA BIBLIOGRÁFICA

Este archivo contiene el estado operativo de la fase. Las definiciones extensas y la secuencia general se conservan en el Plan Maestro.
